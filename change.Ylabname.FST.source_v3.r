...
#print("Manhattan ploting Chromosomewise")
  ...
  for(i in 1:numCHR)
  {
    ...
    ylab = "Fst", main = paste("Chromosome",chm.to.analyze[i],sep=" "),cex.lab=1 , pch=20, xaxs="i", yaxs="i", cex=0.5 )
#abline(h=bonferroniCutOff,col="forestgreen")
abline(h=4,col="forestgreen")
...
#y.lim= 0.9
y.lim=round(max(GI.MP[,3]),2)
  pdf(paste(name.of.trait,".Genomewise.pdf" ,sep = ""), width = 10, height=3.5)
...
plot(y~x,ylab=expression(italic(paste("F"[st],sep = ""))),  ylim=c(0,y.lim), xaxs="i", yaxs="i" ,type = "p", lwd=1.50, pch=20, cex.axis=0.5, cex.lab=1.0 ,col=mycols[z], axes=FALSE,  lty=1, cex=0.35, cex.main=2.0, xlab="Chromosome")
...
#abline(h=0.2,col="green",, lwd=0.5)
